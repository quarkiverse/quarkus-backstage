...
public class BaseFluent<F extends Fluent<F>> implements Fluent<F>, Visitable<F> {
    ...
    public static <T> VisitableBuilder<T, ?> builderOf(T item) {
        ...
        try {
            ...
            try {
                return (VisitableBuilder<T, ?>) Class.forName(item.getClass().getName() + "Builder")
                        .getConstructor(item.getClass())
                        .newInstance(item);
            } catch (Exception e1) {
    ...
    public static <T> List<T> build(List<? extends io.quarkus.backstage.model.builder.Builder<? extends T>> list) {
        return list == null ? null : list.stream().map(Builder::build).collect(Collectors.toList());
    }

    public static <T> Set<T> build(Set<? extends io.quarkus.backstage.model.builder.Builder<? extends T>> set) {
    ...
    public F accept(io.quarkus.backstage.model.builder.Visitor... visitors) {
    ...
    public F accept(List<Entry<String, Object>> path, io.quarkus.backstage.model.builder.Visitor... visitors) {
        return accept(path, "", visitors);
    }

    public F accept(List<Entry<String, Object>> path, String currentKey,
            io.quarkus.backstage.model.builder.Visitor... visitors) {
