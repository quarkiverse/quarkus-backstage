...
When the configuration is exposed, the ArgoCD step is guarded by an `if: ${{ parameters.argocd.enabled }}` condition, allowing users to optionally skip the ArgoCD resource creation when scaffolding a project from the template.
